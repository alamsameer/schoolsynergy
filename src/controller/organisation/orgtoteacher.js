...
export const addClassToOrg = async (req, res) => {
  ...
  try {
    const isClass = await ClassModel.findOne({
      name: classToAdd,
      organisationId,
    });
    ...
    res
      .status(409)
      .json({ message: "class alredy can't create same class twice " });
  } catch (error) {
...
export const getAllClassOfOrganisation = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "getAllClass.length()",
      getAllClass.length,
      getAllClass.length < 1
    );
    if (getAllClass.length < 1) {
      return res
        .status(404)
        .json({ message: "you have not created any class" });
    }
    res.status(200).json({
...
export const assignSubjectToTeacher = async (req, res) => {
  ...
  try {
    ...
    if (!existingAssociation) {
      const orgclass = await ClassModel.findOneAndUpdate(
        { _id: classId, organisationId: organisationId },
        { $push: { subjectTeacher: { teacher: teacherId, subject: subjectId } } },
        {new:true}
      );
      return res
        .status(201)
        .json({ message: "Subject assigned to teacher", orgclass });
    }
