...
export const addQuestionToQuizById=async (req,res)=>{
    const {quizId,teacherId,questiontoAdd,answer,options}=req.body;
    try{
        const question=await questionModel.create({quizId,teacherId,question:questiontoAdd,answer,options})
        const updatedQuiz=await quizModel.findOneAndUpdate({_id:quizId},{$push:{questions:question}})
